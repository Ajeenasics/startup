...
const registerMentor = async (req, res) => {
  try {
    ...
    // Check for existing mentor by contact
...
const editMentorById = async (req, res) => {
  ...
  try {
    const data = await Mentor.mentors.findByIdAndUpdate(
      req.params.id,
      updateData,
      { new: true }
    );
...
const loginMentor = async (req, res) => {
  ...
  try {
    const user = await Mentor.mentors.findOne({ email });
...
const mentorViewTutorialByMentorId = (req, res) => {
  Mentor.mentorTutorial
    .findById({ mentorId: req.params.id })
    .exec()
    .then((data) => {
      res.status(200).json({
        msg: "Data obtained successfully",
